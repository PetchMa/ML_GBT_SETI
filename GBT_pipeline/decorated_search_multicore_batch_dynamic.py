...
def screening(data, labels, index):
    ...
    if fit > METRIC[index]:
        ...
        return False
def extra_screening(data, labels, index):
    fit = silhouette_score(data,labels)
    if fit > METRIC[index]:
        return True, fit
    else:
...
def batch_compute_parallel(result, cadence_length,WINDOW_SIZE,index,freq_ranges, load,n):
    solutions = []
    for el in load[n]:
        solutions.append(compute_parallel(result, cadence_length,WINDOW_SIZE,index,freq_ranges,  el))
...
def compute_parallel(result, cadence_length,WINDOW_SIZE,index,freq_ranges, n):
    ...
    if strong_cadence_pattern(labels):
        if screening(result[n*6: (n+1)*6, : ], labels, index):
            screen_flag, fit = extra_screening(result[n*6: (n+1)*6, : ], labels, index)
...
def classification_data(target_name,cadence, model, out_dir, iterations=6):
    ...
    for index in range(iterations):
        ...
        with Pool(CORES) as p:
            candidates = p.map(functools.partial(batch_compute_parallel, result, cadence_length,WINDOW_SIZE,index, freq_ranges, load), 
                                range(CORES))
    ...
    df = pd.DataFrame(final_set, columns =['start_freq', 'end_freq', 'fit'], dtype = float)
