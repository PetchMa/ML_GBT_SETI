...
model = model_load("../test_bench/VAE-BLPC1-ENCODER_compressed_512v6-6.h5")
...
for i in range(724, len(headers)):
    ...
    result = classification_data(str(col), cadence_set, model,forest_set, "result_BLPC2/", iterations=16)
    print("time: execution: "+str(time.time()-start))
    
    pd.DataFrame(cadence_set).to_csv("result_BLPC2/"+str(col)+"_directory.csv")
    print("Memory used")
