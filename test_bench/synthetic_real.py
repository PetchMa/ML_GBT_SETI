...
def create_false(plate, snr_base=300, snr_range=10, factor = 1):
    ...
    if choice > 0.66:
        ...
        total[5,:,:] = cadence[80:96,:]
    elif  choice > 0.33 and choice < 0.66 :
        total = np.zeros((6,plate.shape[2],plate.shape[3] ))
        
        mean = np.mean(plate[int(plate.shape[0]*random()),0,: ,:])
        std =  np.std(plate[int(plate.shape[0]*random()),0,: ,:])
        # print(mean, std)
        total[0] = np.random.normal(loc=mean, scale = std, size = (1,plate.shape[2],plate.shape[3]  ))
        
        mean = np.mean(plate[int(plate.shape[0]*random()),0,: ,:])
        std =  np.std(plate[int(plate.shape[0]*random()),0,: ,:])
        # print(mean, std)
        total[1] = np.random.normal(loc=mean, scale = std, size = (1,plate.shape[2],plate.shape[3]  ))
        
        mean = np.mean(plate[int(plate.shape[0]*random()),0,: ,:])
        std =  np.std(plate[int(plate.shape[0]*random()),0,: ,:])
        # print(mean, std)
        total[2] = np.random.normal(loc=mean, scale = std, size = (1,plate.shape[2],plate.shape[3]  ))
        
        mean = np.mean(plate[int(plate.shape[0]*random()),0,: ,:])
        std =  np.std(plate[int(plate.shape[0]*random()),0,: ,:])
        # print(mean, std)
        total[3] = np.random.normal(loc=mean, scale = std, size = (1,plate.shape[2],plate.shape[3] ))
        
        mean = np.mean(plate[int(plate.shape[0]*random()),0,: ,:])
        std =  np.std(plate[int(plate.shape[0]*random()),0,: ,:])
        # print(mean, std)
        total[4] = np.random.normal(loc=mean, scale = std, size = (1,plate.shape[2],plate.shape[3] ))
        
        mean = np.mean(plate[int(plate.shape[0]*random()),0,: ,:])
        std =  np.std(plate[int(plate.shape[0]*random()),0,: ,:])
        # print(mean, std)
        total[5] = np.random.normal(loc=mean, scale = std, size = (1,plate.shape[2],plate.shape[3] ))
    else:
        index = int(plate.shape[0]*random())
        total = plate[index,:,:,:]
    return total
