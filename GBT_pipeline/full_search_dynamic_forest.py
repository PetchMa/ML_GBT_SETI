...
# df = pd.read_csv('../data_archive/fine_cadences_formated.csv')
...
for i in range(0, len(headers)):
    ...
    result = classification_data(str(col), cadence_set, model,forest_set, "result/", iterations=16)
    print("time: execution: "+str(time.time()-start))
    
    pd.DataFrame(cadence_set).to_csv("result/"+str(col)+"_directory_"+str(i)+".csv")
    print("Memory used")
