...
def new_frame(mean = 58348559, snr_power = 1, WIDTH_BIN = 4096):
    snr=(random()*10+200)
...
def new_cadence(data, snr, WIDTH_BIN = 4096):
    ...
    drift = -1*(1/slope)
    # witdh = random()*25+5
    witdh = random()*50+abs(drift)*18./1
    # witdh = abs(drift)*18
...
def create_true(plate,  snr_base=300, snr_range=10, factor=1, index=1, WIDTH_BIN = 4096):
...
def create_true_faster(plate,  snr_base=300, snr_range=10, factor=1, index=1, WIDTH_BIN = 4096):
...
def create_true_single_shot(plate, snr_base=10, snr_range=5, factor = 1, index=1, WIDTH_BIN = 4096):
    index = int(plate.shape[0]*random())
    total = np.zeros((6,plate.shape[2],plate.shape[3]  ))
    base = plate[index,:,:,:]
    data = np.zeros((96,WIDTH_BIN))
...
# @jit(nopython=True)
def create_false(plate, snr_base=300, snr_range=10, factor = 1, index=1, WIDTH_BIN = 4096):
    choice = random()
    if choice > 0.5:
        index = int(plate.shape[0]*random())
        total = np.zeros((6,plate.shape[2],plate.shape[3] ))
...
def create_full_cadence(function, samples, plate, snr_base=300, snr_range=10, factor=1, index=1, WIDTH_BIN = 4096):
    data = np.zeros((samples,6,16,WIDTH_BIN))
    for i in prange(data.shape[0]):
        data[i,:,:,:] = function(plate, snr_base=snr_base, snr_range=snr_range, factor=factor, WIDTH_BIN=WIDTH_BIN) 
    return data
...
def create_full_cadence_multicore(function, samples, plate, snr_base=300, snr_range=10, factor=1, WIDTH_BIN = 4096):
    CORES = 10
    with Pool(CORES) as p:
        data = p.map(functools.partial(function, plate, snr_base, snr_range, factor, WIDTH_BIN), 
                                range(samples))
        #     result.append(data)
        # print(data)
    data = stacking(data)
