...
# model_name = "VAE-BLPC1-ENCODER_compressed_512v6-6"
model_name = "../test_bench/VAE-BLPC1-ENCODER_compressed_512v13-0"

model = model_load(model_name+".h5")
# forest = joblib.load('random_forest_10000_'+str(freq_interval)+'.joblib')
# forest = forest_primer(model, plate_train)
# joblib.dump(forest, "../test_bench/random_forest_1000_v6.joblib")
...
plate = np.load('../../../../../../../datax/scratch/pma/real_filtered_LARGE_testHIP83043.npy')

thresh=0.5

FACTOR = 1
snr_range=2
snr_list = np.arange(20,80, snr_range).tolist()
results = [] 
for snr in snr_list:
    print(snr)
...
df.to_csv(model_name+'onband_full_test_forest_'+str(thresh)+'.csv')
